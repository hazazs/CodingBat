...
public class CodingBat {
    ...
    have that value, and set the key "a" to have the value "". Basically "b" is a bully, taking the
    ...
    mapShare
    Modify and return the given map as follows: if the key "a" has a value, set the key "b" to
    have that same value. In all cases remove the key "c", leaving the rest of the map
    unchanged.
     */
    ...
    if the key "ice cream" has a value, set that as the value for the key "yogurt" also. If the
    key "spinach" has a value, change that value to "nuts".
     */
    ...
    topping3
    Given a map of food keys and topping values, modify and return the map as follows: if the
    key "potato" has a value, set that as the value for the key "fries". If the key "salad" has a
    ...
    public Map<String, String> mapAB2(Map<String, String> map) {
        boolean isAPresent = map.containsKey("a");
        boolean isBPresent = map.containsKey("b");
        if (isAPresent && isBPresent && map.get("a").equals(map.get("b"))) {
            map.remove("a");
            map.remove("b");
        }
        return map;
